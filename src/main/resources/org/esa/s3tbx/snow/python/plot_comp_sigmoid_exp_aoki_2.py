...
# Aoki data from AK excel file:
with open("F:/olafd/s3snow/from_alex/site_sigma_20170529.csv") as f:
    data = f.read()
data = data.split('\n')

alb001 = [row.split(' ')[1] for row in data]
alb002 = [row.split(' ')[3] for row in data]
alb003 = [row.split(' ')[5] for row in data]
alb004 = [row.split(' ')[7] for row in data]

t2 = np.arange(0.4, 1.2, 0.001)
plt.plot(t2, alb001, 'b')
plt.plot(t2, alb002, 'b')
plt.plot(t2, alb003, 'b')
plt.plot(t2, alb004, 'b')

# plot properties and legends
line1, = plt.plot(t2, alb001, 'b', label='Aoki Data 20170529')
data_legend = plt.legend(handles=[line1], loc=1, prop={'size': 10})
...
plt.title('Comparison of spectral albedo fits with Aoki field data')
plt.grid(True)
plt.savefig("comp_sigmoid_exp_aoki.png")
